...
Deployed on Railway with a managed PostgreSQL add-on.
...
**Pre-deploy command** (runs after build, before the new instance starts):
```
...
On every push to `main`, GitHub Actions runs lint and the test suite. If that passes, Railway pulls the new commit, builds, runs pending migrations, and starts the server.
...
- `NEXT_PUBLIC_APP_URL` — your `*.up.railway.app` domain (used for CSRF origin check)
